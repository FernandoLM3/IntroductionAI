...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .route-ui { display: flex; flex-direction: column; gap: 8px; border-top: 1px solid var(--line); padding-top: 14px; }
    .route-ui label { margin-bottom: 2px; }
    .route-ui button {
    ...
    .route-ui button:hover { background: #2a614a; }
    .route-result { font-size: 12.5px; color: var(--muted); line-height: 1.5; }
    .route-result h2 { margin: 0 0 4px; font-size: 16px; color: var(--ink); }
    .route-result b { color: var(--ink); }
    .route-result .path { font-family: ui-sans-serif, "Helvetica Neue", sans-serif; color: var(--ink); }
    .route-result .note { color: #9a3412; }
...
<body>
  <aside>
    ...
    <div class="route-ui">
      ...
      <input id="from" type="search" list="cities" placeholder="e.g. Tijuana" autocomplete="off" />
      <label for="to">Destination</label>
      <input id="to" type="search" list="cities" placeholder="e.g. Cancún" autocomplete="off" />
      <button id="findroute" type="button">Find route (A*)</button>
      <div class="route-result" id="route">Pick an origin and a destination, then press <b>Find route</b>.</div>
    </div>
  ...
  <script>
    ...
    // ----- A* route finding (JS port of search/astar.py on the Mexico graph) -----
    ...
    for (const e of G.edges) {
      adj[e.source].push([e.target, e.km]);
      adj[e.target].push([e.source, e.km]);
    }
    // Match the Python Graph.neighbors() order (ascending by node id) so the
    // JS port expands states in the same order and returns the same path.
    adj.forEach(list => list.sort((a, b) => a[0] - b[0]));
    const byName = {};
    G.nodes.forEach(n => {
    ...
    G.nodes.forEach(n => {
      ...
      opt.value = n.name + " (" + n.state + ")";
      citiesDatalist.appendChild(opt);
    });

    function mostPopulous(arr) {
      return arr.slice().sort((a, b) => b.population - a.population)[0];
    }
    function resolveCity(query) {
      ...
      if (m) {
        ...
        const state = normalizeName(m[2]);
        const hits = G.nodes.filter(n => normalizeName(n.name) === name && normalizeName(n.state) === state);
        if (hits.length) {
          const chosen = mostPopulous(hits);
          return { node: chosen, alternatives: hits.filter(n => n.id !== chosen.id), exact: true };
        }
      }
      const key = normalizeName(raw);
      let exact = byName[key] || [];
      if (exact.length) {
        const chosen = mostPopulous(exact);
        return { node: chosen, alternatives: exact.filter(n => n.id !== chosen.id), exact: true };
      }
      let contains = G.nodes.filter(n => normalizeName(n.name).includes(key));
      if (contains.length) {
        const chosen = mostPopulous(contains);
        return { node: chosen, alternatives: contains.filter(n => n.id !== chosen.id), exact: false };
      }
      let prefix = G.nodes.filter(n => normalizeName(n.name).startsWith(key));
      if (prefix.length) {
        const chosen = mostPopulous(prefix);
        return { node: chosen, alternatives: prefix.filter(n => n.id !== chosen.id), exact: false };
      }
      return null;
    }

    // Minimal binary min-heap. Compares (f, counter, id) lexicographically so
    // ties on f(n) break exactly like Python's heapq (f, counter, node).
    class MinHeap {
      constructor() { this.a = []; }
      less(x, y) {
      ...
      push(v) {
        ...
        while (i > 0) { const p = (i - 1) >> 1; if (!this.less(a[i], a[p])) break; [a[p], a[i]] = [a[i], a[p]]; i = p; }
      }
      pop() {
    ...
    function astar(startId, goalId) {
      const goal = G.nodes[goalId];
      const h = (id) => haversineKm(G.nodes[id].lat, G.nodes[id].lon, goal.lat, goal.lon);
      const heap = new MinHeap();
      let counter = 0;
      heap.push([h(startId), counter++, startId, 0]);
      const bestG = { [startId]: 0 };
      const cameFrom = {};
      const explored = new Set();
      let expanded = 0, generated = 1, maxFrontier = 1;
      while (heap.size) {
        const [f, , id, g] = heap.pop();
        if (explored.has(id)) continue;
        if (id === goalId) {
          const path = []; let cur = goalId;
          while (cur !== startId) { path.push(cur); cur = cameFrom[cur]; }
          path.push(startId); path.reverse();
          return { found: true, path, cost: g, expanded, generated, maxFrontier };
        }
        explored.add(id); expanded++;
        for (const [nb, km] of adj[id]) {
          const ng = g + km;
          if (explored.has(nb)) continue;
          if (!(nb in bestG) || ng < bestG[nb]) {
            bestG[nb] = ng; cameFrom[nb] = id;
            heap.push([ng + h(nb), counter++, nb, ng]);
            generated++;
            maxFrontier = Math.max(maxFrontier, heap.size);
          }
        }
      }
      return { found: false, path: [], cost: 0, expanded, generated, maxFrontier };
    }
    ...
    function setGraphDimmed(dim) {
      edgeLayer.style.opacity = dim ? "0.18" : "1";
      nodeLayer.style.opacity = dim ? "0.35" : "1";
      labelLayer.style.opacity = dim ? "0.35" : "1";
    }

    function renderRoute(pathIds) {
      while (routeLayer.firstChild) routeLayer.removeChild(routeLayer.firstChild);
      setGraphDimmed(pathIds.length > 0);
      for (let i = 0; i < pathIds.length - 1; i++) {
        const a = G.nodes[pathIds[i]], b = G.nodes[pathIds[i + 1]];
        const [x1, y1] = project(a.lon, a.lat);
        const [x2, y2] = project(b.lon, b.lat);
        const line = document.createElementNS(NS, "line");
        ...
        line.setAttribute("stroke", "#1f4d3a"); line.setAttribute("stroke-width", "2.6");
        line.setAttribute("stroke-opacity", "0.9"); line.setAttribute("stroke-linecap", "round");
        routeLayer.appendChild(line);
      }
      for (const id of pathIds) {
        const n = G.nodes[id]; const [x, y] = project(n.lon, n.lat);
        const c = document.createElementNS(NS, "circle");
        c.setAttribute("cx", x.toFixed(2)); c.setAttribute("cy", y.toFixed(2));
        c.setAttribute("r", (radius(n.population) + 1.8).toFixed(2));
        c.setAttribute("fill", "none"); c.setAttribute("stroke", "#1f4d3a");
    ...
    function fmtKm(km) {
      return km.toLocaleString("en-US", { minimumFractionDigits: 0, maximumFractionDigits: 1 });
    }
    function shortPath(pathIds, head, tail) {
      head = head || 5; tail = tail || 5;
      const names = pathIds.map(id => G.nodes[id].name);
      if (names.length <= head + tail) return names.join(" → ");
      return names.slice(0, head).join(" → ") + " → … (" + (names.length - head - tail) + " more) … → " + names.slice(-tail).join(" → ");
    }

    function runRoute() {
      const routeEl = document.getElementById("route");
      const from = resolveCity(document.getElementById("from").value);
      const to = resolveCity(document.getElementById("to").value);
      if (!from) { routeEl.innerHTML = "<h2>Route</h2><p class=\"note\">Origin not found.</p>"; renderRoute([]); return; }
      if (!to) { routeEl.innerHTML = "<h2>Route</h2><p class=\"note\">Destination not found.</p>"; renderRoute([]); return; }
      const notes = [];
      if (from.alternatives.length) {
        notes.push("Ambiguous origin \"" + from.node.name + "\": chose the most populous (" + from.node.state + "). Alternatives: " +
          from.alternatives.map(n => n.name + " (" + n.state + ")").join(", ") + ".");
      }
      if (to.alternatives.length) {
        notes.push("Ambiguous destination \"" + to.node.name + "\": chose the most populous (" + to.node.state + "). Alternatives: " +
          to.alternatives.map(n => n.name + " (" + n.state + ")").join(", ") + ".");
      }
      const res = astar(from.node.id, to.node.id);
      renderRoute(res.path);
      const noteHtml = notes.length ? "<p class=\"note\">" + notes.join("<br>") + "</p>" : "";
      if (!res.found) {
        routeEl.innerHTML = "<h2>Route (A*)</h2><p><b>Status:</b> failure · expanded " + res.expanded + " nodes.</p>" + noteHtml;
        return;
      }
      const hops = res.path.length - 1;
      routeEl.innerHTML =
        "<h2>Route (A*)</h2>" +
        "<p><b>Status:</b> success · <b>Heuristic:</b> haversine straight-line distance<br>" +
        "<b>" + from.node.name + "</b> → <b>" + to.node.name + "</b><br>" +
        "<b>Cost:</b> " + fmtKm(res.cost) + " km · <b>Depth:</b> " + hops + " hops<br>" +
        "<b>Expanded:</b> " + res.expanded + " nodes</p>" +
        "<p class=\"path\">" + shortPath(res.path) + "</p>" + noteHtml;
    }

    document.getElementById("findroute").addEventListener("click", runRoute);
    document.getElementById("from").addEventListener("keydown", e => { if (e.key === "Enter") runRoute(); });
    document.getElementById("to").addEventListener("keydown", e => { if (e.key === "Enter") runRoute(); });
